...
test('homepage "Where we\'d start" panel links to the checklist under situation 1 only (EN and DE)', async ({ page }) => {
  await page.goto('/');
  ...
  await page.locator('.situation-card').nth(1).click();
  await expect(link).toHaveCount(0);
  await page.locator('.situation-card').nth(2).click();
  await expect(link).toHaveCount(0);
  await page.locator('.situation-card').nth(0).click();
